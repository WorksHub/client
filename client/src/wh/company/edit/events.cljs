...
(reg-event-fx
  ...
  (fn [{db :db} [yt-id _resp]]
    {:db (update db
                 ::edit/videos
                 #(map (fn [v] (if (= yt-id (:youtube-id v))
                                 (dissoc v :loading?)
                                 v)) %))
     :dispatch [:error/set-global "This video title could not be fetched"]}))
  ...
  (fn [{db :db} [yt-url]]
    (if-let [yt-id (yt-url->yt-id yt-url)]
      (let [thumbnail (str "https://img.youtube.com/vi/" yt-id "/hqdefault.jpg")
            videos (conj (::edit/videos db) {:youtube-id yt-id
                                             :thumbnail thumbnail})
            local-videos (conj (::edit/videos db) {:youtube-id yt-id
                                                   :thumbnail thumbnail
                                                   :loading? true})]
        {:db (assoc db ::edit/videos local-videos)
         ...
         :dispatch [::fetch-video-title yt-id]})
      {:db (assoc db ::edit/video-error :invalid-url)})))

(reg-event-fx
  ::add-video-failure
  company-interceptors
  (fn [{db :db} [yt-id _resp]]
    {:db (update db ::edit/videos #(remove-video % yt-id))
     :dispatch [:error/set-global "This video could not be added"]}))

(reg-event-fx
  ::update-video
  company-interceptors
  (fn [{db :db} [{:keys [youtube-id thumbnail description] :as video}]]
    (let [videos (map (fn [v] (if (= (:youtube-id v) youtube-id)
                                video
                                v)) (::edit/videos db))]
      {:db (assoc db ::edit/videos videos)
       :graphql {:query (update-company-mutation-with-fields [[:videos [:youtubeId :thumbnail :description]]])
                 :variables {:update_company
                             {:id (::edit/id db)
                              :videos (cases/->camel-case videos)}}
...
(reg-event-fx
  ...
  company-interceptors
  (fn [{db :db} [youtube-id]]
    (let [videos (remove-video (::edit/videos db) youtube-id)]
      {:db (assoc db ::edit/videos videos)
       :graphql {:query (update-company-mutation-with-fields [[:videos [:youtubeId :thumbnail :description]]])
                 :variables {:update_company
                             {:id (::edit/id db)
