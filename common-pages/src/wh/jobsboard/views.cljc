...
(defn header
  ...
  ([{:keys [preset-search? searching?]}]
   (let [{:keys [title subtitle _description]} (<sub [::subs/header-info])]
     [:div
...
(defn jobs-board [{:keys [route params query-params view-type preset-search?]}]
...
(defn jobsboard-page []
  (let [logged-in?             (<sub [:user/logged-in?])
    ...
    [:div (merge {:class "main"}
     ...
     [:div.search-results
      [navigation view-type params query-params logged-in?]
...
(defn preset-search-page []
  (let [view-type           (<sub [::subs/view-type])
        ...
        preset-search?      true]
    [:div.main
     [header {:preset-search? preset-search?
              :searching?     searching?}]
